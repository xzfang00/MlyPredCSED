...
n11=length(b11);
for i11=1:n11 
   b11{1,i11}(x11)=''; % 去掉中间赖氨酸
end

M1=length(b1{1,1});  % 去掉中间赖氨酸 49长度变成48
M11=length(b11{1,1}); % 去掉中间赖氨酸 49长度变成48

F1=zeros(20^3,M1-2);
F11=zeros(20^3,M11-2);
for m1=1:n1   % 对于第一类中的每一条序列
    for j1=1:M1-2  % 从第1个位置一致遍历到 48-2个位置
        t1=b1{1,m1}(j1);  % 第m1条序列j1位置元素
        k1=strfind(AA,t1);
        t11=b1{1,m1}(j1+1); % 第m1条序列j1+1位置元素
        k11=strfind(AA,t11);
        t12=b1{1,m1}(j1+2); % 第m1条序列j1+2位置元素
        k12=strfind(AA,t12);
        F1(400*(k1-1)+20*(k11-1)+k12,j1)=F1(400*(k1-1)+20*(k11-1)+k12,j1)+1;
        % 该三联氨基酸频率+1
    end
end
F1=F1/n1;  % F该三联氨基酸频率/第一类中的序列数


for m11=1:n11  % 对于 除了第一类中的每一条序列
    for j11=1:M11-2  % 从第1个位置一致遍历到 48-2个位置
        t1=b11{1,m11}(j11);
        k1=strfind(AA,t1);
        t11=b11{1,m11}(j11+1);
        k11=strfind(AA,t11);
...
F11=F11/n11; % FF该三联氨基酸频率/ 除了第一类中的序列数
...
PSTAAP1=zeros(n1,M1-2);  % 对于第一类的特征
for m1=1:n1  % 对于第一类的每条序列
    for j1=1:M1-2  % 遍历1到48-2个位置
        t1=b1{1,m1}(j1);
        k1=strfind(AA,t1);
        t11=b1{1,m1}(j1+1);
        k11=strfind(AA,t11);
...
%allData = cat(1, PSTAAP1, PSTAAP2, PSTAAP3, PSTAAP4, PSTAAP5, PSTAAP6, PSTAAP7, PSTAAP8, PSTAAP9, PSTAAP10, PSTAAP11);
%save("Fr_train.mat","Fr")
save("PSTAAP_train.mat","PSTAAP1","PSTAAP2","PSTAAP3","PSTAAP4","PSTAAP5","PSTAAP6","PSTAAP7","PSTAAP8","PSTAAP9","PSTAAP10","PSTAAP11")